...
def _question_admin_view(q: Question, source: str, validation: dict | None) -> dict:
    ...
    try:
        pairs = json.loads(q.matching_pairs or "[]") if fmt == "matching" else []
...
async def _persist_generated(
    ...
    *,
    ...
    if fmt == "true_false":
        fields = {
            "option_a": "Đúng", "option_b": "Sai", "option_c": "—", "option_d": "—",
            "correct_answer": str(payload.get("correct_answer", "A")).upper()[:1] or "A",
            "guessing_c": 0.5,
        }
        if fields["correct_answer"] not in ("A", "B"):
            raise ValueError("true_false: correct_answer phải là A hoặc B")
    elif fmt == "short_answer":
        answer_text = str(payload.get("answer_text", "")).strip()
        if not answer_text:
            raise ValueError("short_answer: thiếu answer_text")
        fields = {
            "option_a": "—", "option_b": "—", "option_c": "—", "option_d": "—",
            "correct_answer": "A", "guessing_c": 0.05, "answer_text": answer_text,
        }
    elif fmt == "matching":
        pairs = payload.get("pairs") or payload.get("matching_pairs") or []
        pairs = [
        ...
        if len(pairs) < 3:
            raise ValueError("matching: cần ít nhất 3 cặp")
        fields = {
            "option_a": "—", "option_b": "—", "option_c": "—", "option_d": "—",
            "correct_answer": "A", "guessing_c": 0.05,
            "matching_pairs": json.dumps(pairs, ensure_ascii=False),
        }
    else:  # mcq
        correct = str(payload.get("correct_answer", "")).upper()[:1]
        if correct not in ("A", "B", "C", "D"):
            raise ValueError("mcq: correct_answer phải thuộc A/B/C/D")
        fields = {
            "option_a": str(payload.get("option_a", "")).strip(),
            "option_b": str(payload.get("option_b", "")).strip(),
            "option_c": str(payload.get("option_c", "")).strip(),
            "option_d": str(payload.get("option_d", "")).strip(),
            "correct_answer": correct,
            "guessing_c": max(0.0, min(0.35, float(payload.get("guessing_c", 0.25) or 0.25))),
        }
        ...
        for bloom, q_need in quota.items():
            for _ in range(max(0, q_need)):
                ...
                try:
                    ...
                    warnings.append(f"Sinh câu {fmt}/{bloom} thất bại: {str(exc)[:150]}")
